final class VerticalStrip: UIView, UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    func commonInit() {
        ...
        upButton.addTarget(self, action: #selector(didTapUpButton(_:)), forControlEvents: .TouchUpInside)
        addSubview(upButton)
        ...
        downButton.addTarget(self, action: #selector(didTapDownButton(_:)), forControlEvents: .TouchUpInside)
    ...
    func didTapUpButton(button: UIButton) {
    ...
    func didTapDownButton(button: UIButton) {
        let currentItemIndex = Int(collectionView.contentOffset.y / collectionView.frame.height)
